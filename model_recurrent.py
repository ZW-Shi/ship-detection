'''
CORNet without channel attention
'''

from utils.parse_config import *
from models_config import Upsample, EmptyLayer, YOLOLayer
from attention_module import PositionAttentionModule,ChannelAttentionModule,ChannelAttention
...
class CAR_Block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, times=1):
        ...
        # self.attention = PositionAttentionModule(out_channels)
        ...
        # need BatchNorm for each time step for training to work well   每次循环后都需要做一次BatchNormsize
        for t in range(self.times):
            setattr(self, f'norm2_{t}', nn.BatchNorm2d(out_channels * self.scale))
            setattr(self, f'norm3_{t}', nn.BatchNorm2d(out_channels))
...
class Darknet(nn.Module):
    ...
    def load_cornet_weights(self, weight_path):
        ...
        pretrained_dict = torch.load('F:\\huiyi_jiance\\weights\\new_cornet_s.pth')
